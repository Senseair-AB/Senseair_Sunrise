 ...
 *******************************************************************************
   ...
   @file        s12_single.ino
   @brief       S12 CO2 sensor single measurement mode example for I2C and Modbus.
 ...
 * In single mode, measurement period does not affect the sensor.
 * It is used by this host application to determine how often to send
...
constexpr bool PRESSURE_COMP_ENABLED = false; /**< Enable/disable pressure compensation */
...
void setup()
{
        ...
        if (error == SunriseError::NO_ERROR)
        {
            ...
            if (CHANGE_MEASUREMENT_CONFIGURATION)
            {
                    ...
                    (baseConfiguration.measSamples != MEASUREMENT_SAMPLES))
                {
                    ...
                    baseConfiguration.measSamples = MEASUREMENT_SAMPLES;
                    error                         = sensor.setSensorConfig(baseConfiguration);
    ...
    do
    {
        ...
        error = sensor.initialSingleMeasurement(sunriseMeasurement, sunriseState, MEASUREMENT_SAMPLES);
        if (error == SunriseError::NO_ERROR)
        {
            ...
            Serial.println(sensor.getABCTimeFromState(sunriseState));
...
void loop()
{
    ...
    if (elapsed(currentTimeStamp, measurementTimeStamp, sunriseMeasurementPeriodMs))
    {
        ...
        SunriseMeasurement sunriseMeasurement = {0, 0, 0u};
        error = sensor.readSingleMeasurement(sunriseMeasurement, sunriseState, MEASUREMENT_SAMPLES);
        if (error == SunriseError::NO_ERROR)
        {
        ...
        /* In Single measurement mode, the RDYN pin changes when the measurement is finished. The EEPROM write event
         * takes 275ms*/
        delay(S12_CALIBRATION_TIME_SINGLE_MODE_MS);
        Serial.println("\nWaiting for next measurement...\n");
        sensor.disable();
    }
    ...
    if (elapsed(currentTimeStamp, abcTimeStamp, HOUR_INTERVAL_MS))
    {
        if (ABC_ENABLED)
        {
            sensor.incrementABCTimeInState(sunriseState);
